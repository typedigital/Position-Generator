...
async function getUserIdByEmail(
  ...
  const response = await fetch(
    `${baseUrl}/users/find?term=${encodeURIComponent(email)}&api_token=${apiToken}`,
  );
...
async function getOrCreatePerson(
  ...
  const searchRes = await fetch(
    `${baseUrl}/persons/search?term=${encodeURIComponent(email)}&fields=email&api_token=${apiToken}`,
  );
